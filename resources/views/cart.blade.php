            ...
            <div class="col-md-10 offset-md-1">
                                    ...
                                    <td>
                                        <div class="quantity-input-contain">
                                            <input type="number" class="quantity-input" value="{{ $item->qty }}"
                                                data-id="{{ $item->rowId }}"
                                                data-productQuantity="{{ $item->model->quantity }}" min="1">
                ...
                @else
                    <div class="alert alert-info">
                        <h4 class="lead">No items in the cart <a class="btn custom-border-n"
                                href="{{ route('shop.index') }}">Continue shopping</a></h4>
                    </div>
                @endif
                <hr>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('.quantity-btn').on('click', function() {
                ...
                const id = $(this).siblings('.quantity-input').attr('data-id');

                const productQuantity = $(this).siblings('.quantity-input').attr('data-productQuantity');

                const currentQuantity = parseInt($(this).siblings('.quantity-input').val());

                if ($(this).hasClass('quantity-minus') && currentQuantity > 1) {
                    $(this).siblings('.quantity-input').val(currentQuantity - 1);
                } else if ($(this).hasClass('quantity-plus') && currentQuantity < 10) {
                    $(this).siblings('.quantity-input').val(currentQuantity + 1);
                }

                const newQuantity = $(this).siblings('.quantity-input').val();
